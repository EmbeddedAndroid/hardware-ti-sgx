...
//	printk (" fb_offset: %lu \n", fb_offset);
	...
	fbdev = ofbi->fbdev;
        ...
        memcpy ( &var, &framebuffer->var, sizeof(struct fb_var_screeninfo));

        var.yoffset = fb_offset/framebuffer->fix.line_length;
	if (framebuffer->fbops->fb_pan_display != NULL) {
//		printk ("var.yoffset: %u \n", var.yoffset);
		framebuffer->fbops->fb_pan_display (&var, framebuffer);
	}
...
#endif
void OMAPLFBWaitForVSync(OMAPLFB_SWAPCHAIN *psSwapChain)
{
...
#if 0
	        if (lcd_mgr )
			lcd_mgr->wait_for_vsync(lcd_mgr);
#endif
}
